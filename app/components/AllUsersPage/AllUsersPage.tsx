...
async function getData(key:any){
    try{
        console.log("key");
        console.log(key)
        const res2 = await Promise.all([fetch(`${process.env.NEXT_PUBLIC_REACT_SERVER_API}/users/fields/alldata?fields=true`), fetch(`${process.env.NEXT_PUBLIC_REACT_SERVER_API}/users/currentUser/alldata?pageIndex=${key?.queryKey[0]}&rowsPerPage=20&searchKey=${key?.queryKey[1]}&searchQuery=${key?.queryKey[2]}`)]);
...
function AllUsersPage() {
    ...
    const getFieldData = async ()=>{
        try{
            const res = await fetch(`${process.env.NEXT_PUBLIC_REACT_SERVER_API}/users/fields/alldata?fields=true`);
  ...
  return (
    <>
      ...
      errorMessage && 
      <>
        <div role="alert" className="alert alert-error fixed left-0 z-50 right-0 top-[0vh] w-[90vw] justify-self-center self-center gap-1 flex-row prompt-anime mx-auto">
        ...
        className='flex flex-col gap-3 w-full justify-center items-center fixed left-0 right-0 top-11 pt-8 pb-1 bg-base-100 z-20'>
            <div className='flex flex-row justify-between w-full px-5 bg-base-100'>
                ...
                <h1 className='text-3xl font-bold self-center justify-center text-center'>All Users</h1>
...
AllUsersPage.displayName = "AllUsersPage";
