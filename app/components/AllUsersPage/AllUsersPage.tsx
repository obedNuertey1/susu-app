...
async function getData(key:any){
    try{
        console.log("key");
        console.log(key)
        const res2 = await Promise.all([fetch(`${process.env.REACT_SERVER_API}/users/fields/alldata?fields=true`), fetch(`${process.env.REACT_SERVER_API}/users/currentUser/alldata?pageIndex=${key?.queryKey[0]}&rowsPerPage=20&searchKey=${key?.queryKey[1]}&searchQuery=${key?.queryKey[2]}`)]);
...
function AllUsersPage() {
    ...
    const getFieldData = async ()=>{
        try{
            const res = await fetch(`${process.env.REACT_SERVER_API}/users/fields/alldata?fields=true`);
